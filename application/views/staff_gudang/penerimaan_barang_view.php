...
<style>
  ...
  .rcv-index-page .toolbar-card{
    background:#fff; border:1px solid #edf0f1; border-radius:12px;
    padding:14px 16px; box-shadow:0 2px 10px rgba(15,42,41,.04); margin-bottom:18px;
  }

  .rcv-index-page .kpi-card{ display:flex; align-items:center; gap:12px; }
  .rcv-index-page .kpi-icon{
    width:44px; height:44px; border-radius:10px; flex-shrink:0;
    display:flex; align-items:center; justify-content:center; font-size:1.15rem;
  }
  .rcv-index-page .kpi-icon.bg-total{ background:var(--brand-light); color:var(--brand); }
  .rcv-index-page .kpi-icon.bg-draft{ background:#fff4d6; color:#946c00; }
  .rcv-index-page .kpi-icon.bg-selesai{ background:#dff3e6; color:#1f7a45; }
  .rcv-index-page .kpi-icon.bg-kurang{ background:#fde2e1; color:#b3261e; }

  .rcv-index-page .table-wrap{
    background:#fff; border-radius:12px; border:1px solid #edf0f1;
    box-shadow:0 2px 10px rgba(15,42,41,.04); overflow:hidden;
  }
  ...
  .rcv-index-page table tbody td{
    padding:.8rem 1rem; vertical-align:middle; font-size:.88rem; border-color:#f1f3f2;
  }
  .rcv-index-page table tbody tr:hover{ background:#fafcfb; }

  .rcv-index-page .badge-status{ padding:5px 12px; border-radius:20px; font-weight:600; font-size:.74rem; white-space:nowrap; }
  .rcv-index-page .badge-draft{ background:#fff4d6; color:#946c00; }
  .rcv-index-page .badge-selesai{ background:#dff3e6; color:#1f7a45; }
  .rcv-index-page .badge-kurang{ background:#fde2e1; color:#b3261e; }
  ...
  .rcv-index-page .empty-row td{ padding:48px 20px; text-align:center; color:#8a938d; }
  .rcv-index-page .empty-row i{ font-size:2.2rem; display:block; margin-bottom:10px; color:#c3d0c9; }

  .rcv-index-page .pagination .page-link{ color:var(--brand); border-color:#e5e7eb; }
  .rcv-index-page .pagination .page-item.active .page-link{ background:var(--brand); border-color:var(--brand); }

  @media (max-width:767px){
    .rcv-index-page .table-wrap{ overflow-x:auto; }
    .rcv-index-page .toolbar-card .row > div{ margin-bottom:.5rem; }
  }
</style>
    ...
    <div class="col-6 col-lg-3">
      <div class="kpi-card">
        ...
        <div>
          ...
          <div class="kpi-label">Masih Draft</div>
        </div>
      </div>
    </div>
    <div class="col-6 col-lg-3">
      <div class="kpi-card">
        ...
        <div>
          ...
          <div class="kpi-label">Selesai / Sesuai PO</div>
        </div>
      </div>
    </div>
    <div class="col-6 col-lg-3">
      <div class="kpi-card">
        <div class="kpi-icon bg-kurang"><i class="bi bi-exclamation-triangle"></i></div>
        <div>
          <div class="kpi-value"><?= isset($total_kurang) ? $total_kurang : 0; ?></div>
          <div class="kpi-label">Kurang dari PO</div>
        </div>
      </div>
    </div>
  </div>
    ...
    <?php echo form_open('penerimaan_barang', ['method' => 'get', 'class' => 'row g-2 align-items-end']); ?>
      ...
      <div class="col-6 col-md-2">
        ...
        <select name="status" class="form-select">
          ...
          <option value="kurang" <?= (isset($filter_status) && $filter_status === 'kurang') ? 'selected' : ''; ?>>Kurang dari PO</option>
        </select>
      </div>
            ...
            <?php foreach ($penerimaan_list as $row): ?>
              <?php
                ...
                if ($row->status === 'selesai') {
                    ...
                    $status_label = 'Kurang dari PO';
                }
              ?>
              <tr>
                <td><?= $no++; ?></td>
                <td class="fw-semibold">RCV-<?= html_escape($row->no_penerimaan); ?></td>
